...
class UzsakymasController
{
    ...
    {
        $this->defaultData();
        $this->addData("uzsakymai", $this->getAllMedziagos());
   ...
   {
       $this->defaultData();
       $medziaga = $this->getMedziaga($args['id']);
   ...
   private function getMedziaga($id)
   {
       $query = $this->db->query('SELECT * FROM reklamos_medziagos WHERE id='.$id);
       $query->execute();
       $result = $query->fetch();
...
//                var_dump($postValues);die;
    ...
    {
        $query = $this->db->query('SELECT * FROM reklamos_medziagos');
        $results = $query->fetchAll(PDO::FETCH_ASSOC);
        $all_objects = array();
        foreach ($results as $row){
   ...
   public function loadToFormForAdd($request, $response, $args)
   {
    ...
    public function update($request, $response, $args)
    {
        if($request->isPost()) {
            ...
            $turinys = $postValues["input_turinys"];
            $tipas = $postValues["tipas"];
            $this->updateMaterial($turinys, $tipas, $args["id"]);
    ...
    private function updateMaterial( $turinys, $tipas, $id )
    {
