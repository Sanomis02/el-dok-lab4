...
class MedziagaController
{
    ...
    public function __construct(\Slim\Views\Twig $view, \Slim\Flash\Messages $flash)
    {
        $this->view = $view;
        $this->flash = $flash;

        try {
        $this->duomb = new PDO('mysql:host=animals_mysql;dbname=animals', $_SESSION['vardas'], $_SESSION['slaptazodis']);
        } catch (PDOException $e) {
            $this->addFlashMessage("danger", "Neteisingi duomenys");
            $this->duomb = new PDO('mysql:host=animals_mysql;dbname=animals', 'root', 'rootpassw');
        }
    }
...
//        var_dump($this->getAllSkelbimus());die;
        $this->addData("medziagos", $this->getAllMedziagos());

        $this->addData("successMessage",$this->getFlashMessage("success")[0]);
        $this->view->render($response,'reklamos_medziagos/view_all.twig',$this->getData());
   ...
   {
       $this->defaultData();
       $medziaga = $this->getMedziaga($args['id'], $response);
       $this->addData("medziaga", $medziaga);
       $this->addData("successMessage",$this->getFlashMessage("success")[0]);
       $this->view->render($response,'reklamos_medziagos/edit_one.twig', $this->getData());
       return $response;
   }

   private function getMedziaga($id, $response)
   {
       try
       {
            $query = $this->duomb->query('SELECT * FROM reklamos_medziagos WHERE id='.$id);
              $query->execute();
              $result = $query->fetch();
              $medziaga = new Medziaga($this->db);
              return $medziaga->create($result);
       } catch(PDOException $e) {
...
//        var_dump($_SESSION["paiesk_fragm_sesija"]);die;
    ...
    public function addFromForm($request, $response, $args)
    {
        if($request->isPost()) {
            $postValues = $request->getParsedBody();
            $turinys = $postValues["input_turinys"];
            $tipas = $postValues["tipas"];
            $this->createMedziaga($turinys, $tipas);
            $this->addFlashMessage("success", "Sekmingai uzregistruota");
            return $response->withRedirect('/materials', 301);
        }
    }
...
//                var_dump($postValues);die;
    ...
    private function getAllMedziagos()
    {
        $query = $this->duomb->query('SELECT * FROM reklamos_medziagos');
        $results = $query->fetchAll(PDO::FETCH_ASSOC);
        $all_objects = array();
        foreach ($results as $row){
            $tdo = new Medziaga($this->duomb);
            $all_objects[] = $tdo->create($row);
        }
        return $all_objects;
    }
   ...
   public function loadToFormForAdd($request, $response, $args)
   {
       ...
       $this->addData("successMessage",$this->getFlashMessage("success")[0]);
       $data = $this->getData();
       $this->view->render($response,'reklamos_medziagos\create_new.twig',$data);
       return $response;
   }
    ...
    public function update($request, $response, $args)
    {
        if($request->isPost()) {
            ...
            $turinys = $postValues["input_turinys"];
            $tipas = $postValues["tipas"];
            $this->updateMaterial($turinys, $tipas, $args["id"], $response);
            $this->addFlashMessage("success","Reklamos medziaga atnaujinta");
            return $response->withRedirect("/edit-material/{$args['id']}",301);
        }
    ...
    private function createMedziaga($turinys, $tipas)
    {
        $query= $this->db->prepare("INSERT INTO reklamos_medziagos(turinys, fk_tipas)
          VALUES(:turinys, :tipas)");
        $query->execute([
            "turinys" => $turinys,
            "tipas" => $tipas
        ]);
    ...
    private function updateMaterial( $turinys, $tipas, $id, $response )
    {
        $this->testDBConnection($response);
        $query= $this->db->prepare("UPDATE reklamos_medziagos SET `turinys`=:turinys,
            `fk_tipas`=:tipas WHERE id=". $id);
        $query->execute([
            "turinys" => $turinys,
            "tipas" => $tipas,
        ]);
    ...
    private function testDBConnection($response)
    {
        try {
            $this->db = new PDO('mysql:host=animals_mysql;dbname=animals', $_SESSION['vardas'], $_SESSION['slaptazodis']);
